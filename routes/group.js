...
router.put('/:groupId/members/:memberId/:state', groupCheck, isGroupAdminAuth, groupController.editMemberState);
...
router.post('/:groupId/members/join', groupCheck, hasGroupMember, isNotGroupMemberAuth, groupController.joinMember);

// users can leave a group
// they must not be in the group
// if they are not past the limit of groups that hey can be member of
// the members group count will be released, and the membership relationship will be hidden ('left')
router.put('/:groupId/members/leave', groupCheck, isGroupMemberAuth, groupController.leaveMember);

// group admin user can update certain details within a group (when state is not 'blocked')
// they must be group admin for the defined group
router.put('/:groupId', groupCheck, isGroupAdminAuth, groupValidation, checkValidationResult, groupController.edit);
...
router.get('/:groupId/members', groupCheck, isGroupMemberAuth, searchValidation, groupController.getMembers);
