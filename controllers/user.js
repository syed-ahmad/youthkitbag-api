...
exports.getUser = (req, res, next) => {
  ...
  User
    .findById(userId)
    .then(user => {
      res.status(200).json({ profile: { ...user.profile, _id: user._id }, package: user.package, email: user.email } );
    })
    .catch(err => {
      if (!err.statusCode) {
        err.statusCode = 500;
...
exports.editProfile = (req, res, next) => {
  ...
  User.findById(userId)
    ...
    .then(result => {
      ...
      res.status(201).json({ message: `User profile successfully updated.`, profile: profile });
    })
    .catch(err => {
      if (!err.statusCode) {
        err.statusCode = 500;
